...
#
# Important for using:
#   Run this script as: ./gradlew <task>
#   Or: bash gradlew <task>
#
##############################################################################
...
# This is reliable on all POSIX.1-2007 systems.
...
if "$cygwin" || "$msys" ; then
    ...
    for arg do
        ...
        # temporary variables.  (e.g. if 3 temporaries, "t1 t2 t3 $@" is right,
        # but "t1 t2 t3 a b c $@" is wrong)
        set -- "$arg" "$@"
    done
fi
